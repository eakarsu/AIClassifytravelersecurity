...
// Shared error handler for AI failures
  ...
  async (req, res) => {
    ...
    try {
      ...
      const messages = [
        {
          role: 'system',
          content: SYSTEM_PROMPT + ' Respond with structured analysis. Use markdown headers for sections.'
        },
        {
          role: 'user',
...
- Category: ${category || 'Unknown'}`
      ...
      res.json({ raw_response: aiText, parsed: sections, risk_score: riskScore, alert_id: id || null });
    } catch (error) {
      handleAIError(error, res, 'classify-alert');
    }
  }
);
...
router.post(
  '/assess-risk',
  [
    body('name').notEmpty().withMessage('name (destination) is required').isString().isLength({ max: 200 }),
    body('country').notEmpty().withMessage('country is required').isString().isLength({ max: 100 }),
    body('current_alerts').optional().isString(),
  ],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
    ...
    try {
      ...
      const messages = [
        {
          role: 'system',
          content: SYSTEM_PROMPT + ' Provide detailed, structured risk assessments using markdown headers.'
        },
        {
          role: 'user',
...
- **Recommendations** for travelers`
      ...
      res.json({ raw_response: aiText, parsed: sections, risk_assessment: result.rows[0] });
    } catch (error) {
      handleAIError(error, res, 'assess-risk');
    }
  }
);
...
router.post(
  ...
  [
    ...
    body('description').notEmpty().withMessage('description is required').isString().isLength({ max: 5000 }),
    body('region').optional().isString().isLength({ max: 200 }),
    body('threat_type').optional().isString().isLength({ max: 100 }),
  ],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
    ...
    try {
      ...
      const messages = [
        {
          role: 'system',
          content: SYSTEM_PROMPT + ' Provide detailed, structured threat analyses using markdown headers.'
        },
        {
          role: 'user',
...
- Description: ${description}
- Region: ${region || 'Unknown'}
- Threat Type: ${threat_type || 'Unknown'}
...
6. **Potential Escalation** - likelihood and scenarios`
      ...
      const result = await pool.query(
        `INSERT INTO threat_analyses
         ...
         RETURNING *`,
        [
          title, threat_type || null, region || null, threatLevel, description,
      ...
      res.json({ raw_response: aiText, parsed: sections, threat_analysis: result.rows[0] });
    } catch (error) {
      handleAIError(error, res, 'analyze-threat');
    }
  }
);
...
router.post(
  ...
  [
    body('country').notEmpty().withMessage('country is required').isString().isLength({ max: 100 }),
    body('region').optional().isString().isLength({ max: 200 }),
    body('situation').notEmpty().withMessage('situation is required').isString().isLength({ max: 5000 }),
  ],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
    ...
    try {
      ...
      const messages = [
        {
          role: 'system',
          content: SYSTEM_PROMPT + ' Generate official-style travel advisories with structured sections using markdown headers.'
        },
        {
          role: 'user',
...
5. **Description** - detailed situation overview`
      ...
      res.json({ raw_response: aiText, parsed: sections, advisory: result.rows[0] });
    } catch (error) {
      handleAIError(error, res, 'generate-advisory');
    }
  }
);
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!location) {
        return res.status(400).json({ error: 'region or country is required' });
      }
      ...
      const messages = [
        {
          role: 'system',
          content: SYSTEM_PROMPT + ' Provide comprehensive situation summaries using markdown headers.'
        },
        {
          role: 'user',
...
6. **Areas to Monitor** - specific areas or issues that require ongoing attention`
      ...
      handleAIError(error, res, 'summarize-situation');
    }
  }
);
...
router.post(
  ...
  [
    body('title').notEmpty().withMessage('title is required').isString().isLength({ max: 500 }),
    body('description').notEmpty().withMessage('description is required').isString().isLength({ max: 5000 }),
    body('incident_type').optional().isString().isLength({ max: 100 }),
    body('severity').optional().isString().isLength({ max: 50 }),
    body('location').optional().isString().isLength({ max: 200 }),
    body('country').optional().isString().isLength({ max: 100 }),
    body('affected_travelers_count').optional().isInt({ min: 0 }),
  ],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
    ...
    try {
      ...
      const messages = [
        {
          role: 'system',
          content: SYSTEM_PROMPT + ' Provide detailed incident analysis with actionable response priorities using markdown headers.'
        },
        {
          role: 'user',
...
6. **Follow-up Actions** - medium and long-term actions needed`
      ...
      res.json({ raw_response: aiText, parsed: sections, incident_id: id || null });
    } catch (error) {
      handleAIError(error, res, 'analyze-incident');
    }
  }
);
