  ...
  try {
    ...
    if (!process.env.OPENROUTER_API_KEY) {
      return res.status(503).json({
        ...
        feature: 'gap_ainew_js_scaffold_but_0_mounted_chat_style_ai_endp',
      });
    }
    const payload = req.body || {};
    const userPrompt = `Context (JSON):\n${JSON.stringify(payload).slice(0, 4000)}\n\nTask: ${ 'aiNew.js scaffold but 0 mounted /chat-style AI endpoints' }\n\nReturn JSON: {"summary": string, "actions": [string], "risks": [string], "confidence": number_0_to_1}.`;
    const system = `You are an expert assistant for the feature: 'aiNew.js scaffold but 0 mounted /chat-style AI endpoints'. Respond with strict JSON.`;
    ...
    const out = { feature: 'gap_ainew_js_scaffold_but_0_mounted_chat_style_ai_endp', model: aiResp?.model, parsed, raw: content };
    if (dbPool) {
      try {
        await dbPool.query(
          'INSERT INTO gap_features (feature_key, user_id, input_payload, output_payload) VALUES ($1, $2, $3, $4)',
          ['gap_ainew_js_scaffold_but_0_mounted_chat_style_ai_endp', req.user?.id || null, JSON.stringify(payload), JSON.stringify(out)]
        );
...
router.get('/health', (req, res) => res.json({ feature: 'gap_ainew_js_scaffold_but_0_mounted_chat_style_ai_endp', ok: true, ai_configured: !!process.env.OPENROUTER_API_KEY }));
...
router.get('/history', async (req, res) => {
  ...
  try {
    ...
    const r = await dbPool.query(
      ...
      ['gap_ainew_js_scaffold_but_0_mounted_chat_style_ai_endp']
    );
