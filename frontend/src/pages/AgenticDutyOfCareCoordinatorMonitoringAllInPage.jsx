...
export default function AgenticDutyOfCareCoordinatorMonitoringAllInPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-agentic-duty-of-care-coordinator-monitoring-all-in/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic duty-of-care coordinator monitoring all in-trip employees in real time</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-duty-of-care-coordinator-monitoring-all-in/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
