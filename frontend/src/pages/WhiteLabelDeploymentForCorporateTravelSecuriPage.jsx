...
export default function WhiteLabelDeploymentForCorporateTravelSecuriPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-white-label-deployment-for-corporate-travel-securi/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>White-label deployment for corporate travel security teams</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-white-label-deployment-for-corporate-travel-securi/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
