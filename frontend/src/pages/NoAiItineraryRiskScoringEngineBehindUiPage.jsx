...
export default function NoAiItineraryRiskScoringEngineBehindUiPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-no-ai-itinerary-risk-scoring-engine-behind-ui-page/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No AI itinerary risk-scoring engine behind UI page</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-ai-itinerary-risk-scoring-engine-behind-ui-page/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
