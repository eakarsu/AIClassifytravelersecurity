...
export default function NoDirectHrTravelBookingApiClientConcurEgenPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-no-direct-hr-travel-booking-api-client-concur-egen/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No direct HR/travel-booking API client (Concur, Egencia)</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-direct-hr-travel-booking-api-client-concur-egen/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
