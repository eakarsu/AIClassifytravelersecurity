...
export default function RealTimeGeofenceStreamingWithAutoEscalationPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-real-time-geofence-streaming-with-auto-escalation-/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Real-time geofence streaming with auto-escalation playbooks</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-real-time-geofence-streaming-with-auto-escalation-/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
