...
export default function NoMobileTravelerAppWithSosButtonOnlyApiEnPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-no-mobile-traveler-app-with-sos-button-only-api-en/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No mobile traveler app with SOS button (only API endpoint)</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-mobile-traveler-app-with-sos-button-only-api-en/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
