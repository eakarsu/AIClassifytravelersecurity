...
export default function NoAiGeofenceAnomalyDetectionWiredToTravelerPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-no-ai-geofence-anomaly-detection-wired-to-traveler/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No AI geofence-anomaly detection wired to traveler positions</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-ai-geofence-anomaly-detection-wired-to-traveler/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
